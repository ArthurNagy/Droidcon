...
class MyAgendaAdapter : ViewModelBoundAdapter<SessionItemBinding, SessionItemViewModel>() {
    ...
    override fun bindItem(holder: BindingViewHolder<SessionItemBinding, SessionItemViewModel>, position: Int, payloads: List<Any>) {
        holder.viewModel.title.set(items[position])
    }
